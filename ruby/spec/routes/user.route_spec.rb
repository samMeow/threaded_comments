...
describe UserRoute do
    describe 'get /users/:id' do
        ...
        it 'should return 404 when user not exits' do
            ...
            expect(last_response.body).to eq('{"code":404,"message":"User 2 not found"}')
        end
    end
    ...
    describe 'post /users' do
        ...
        it 'should reject empty body' do
            ...
            expect(last_response.body).to eq('{"code":400,"message":"The property \'#/\' did not contain a required property of \'name\'"}')
        end

        it 'should reject non-string name' do
            post '/users', { name: 123 }.to_json, 'Content-Type' => 'application/json'
            expect(last_response.status).to equal(400)
            expect(last_response.body).to eq("{\"code\":400,\"message\":\"The property '#/name' of type integer did not match the following type: string\"}")
        end

        it 'should accept valid request' do
            post '/users', { name: 'hello' }.to_json, 'Content-Type' => 'application/json'
            expect(last_response.status).to equal(200)
    ...
    describe 'update /users' do
        ...
        it 'should reject not exists users' do
            Models::User.set_dataset(@DB[:users])
            put '/users/1', { name: 'valid' }.to_json, 'Content-Type' => 'application/json'
            expect(last_response.status).to equal(404)
            expect(last_response.body).to eq("{\"code\":404,\"message\":\"User 1 not found\"}")
        end
    end
end
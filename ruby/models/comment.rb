module Models
    class Comment < Sequel::Model(:comments)
        ...
        def to_public
            json_serializer_opts(except: [:path, :parent_path])
            self
        end
        ...
        dataset_module do
            ...
            def order_with_popular
                root_id = Sequel.cast_string(Sequel.function('subpath', :path, 0, 1))
                query = Comment.select(:id, Sequel.as(:score, 'root_score'))
                join(query, id: Sequel.cast(Sequel.function(:ltrim, root_id, '0'), :bigint))
